    ...
    fn mul(self, other: Matrix<T, C0_R1, C1>) -> Matrix<T, R0, C1> {
        match (C0_R1, C1) {
            ...
            _ => {
                ...
                for j in 0..C1 {
                    for i in 0..R0 {
                        output.data[j][i] = self.row(i).dot(Matrix {
                            data: [other.data[j]],
                        });
                    }
                }
...
impl<T: Numeric, const R: usize, const C: usize> Mul<T> for Matrix<T, R, C> {
    ...
    fn mul(self, other: T) -> Self {
        ...
        for i in 0..C {
            for j in 0..R {
                output.data[i][j] = self.data[i][j] * other
            }
        }
        output
    }
...
impl<T: Numeric + Neg<Output = T>, const R: usize, const C: usize> Neg for Matrix<T, R, C> {
    ...
    fn neg(self) -> Self {
        ...
        for i in 0..C {
            for j in 0..R {
                output.data[i][j] = output.data[i][j].neg();
            }
        }
        output
    }
...
impl<T: Numeric, const R: usize, const C: usize> Matrix<T, R, C> {
    ...
    pub fn min(self, other: Self) -> Self {
        ...
        for i in 0..C {
            for j in 0..R {
                output.data[i][j] = self.data[i][j].numeric_min(other.data[i][j])
            }
        }
        output
    }
    ...
    pub fn max(self, other: Self) -> Self {
        ...
        for i in 0..C {
            for j in 0..R {
                output.data[i][j] = self.data[i][j].numeric_max(other.data[i][j])
            }
        }
        output
    }
    ...
    pub fn clamp(self, min: Self, max: Self) -> Self {
        ...
        for i in 0..C {
            for j in 0..R {
                output.data[i][j] = self.data[i][j].numeric_clamp(min.data[i][j], max.data[i][j])
            }
        }
        output
    }
    ...
    pub fn min_component(self) -> T {
        ...
        for i in 0..C {
            for j in 0..R {
                min = self.data[i][j].numeric_min(min)
            }
        }
        min
    }
    ...
    pub fn max_component(self) -> T {
        ...
        for i in 0..C {
            for j in 0..R {
                max = self.data[i][j].numeric_max(max)
            }
        }
        max
    }
...
impl<T: Numeric + NumericAbs, const R: usize, const C: usize> Matrix<T, R, C> {
    pub fn abs(self) -> Self {
        ...
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = self.data[i][j].numeric_abs()
            }
        }
        v
    }
...
impl<T: Numeric + NumericSigNum, const R: usize, const C: usize> Matrix<T, R, C> {
    pub fn signum(self) -> Self {
        ...
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = self.data[i][j].signum_numeric()
            }
        }
        v
    }
...
impl<T: Numeric, const R: usize, const C: usize> Matrix<T, R, C> {
        ...
        T: NumericFloat,
    {
        ...
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = T::ONE / self.data[i][j];
            }
        }
        v
    }
        ...
        T: NumericFloat,
    {
        ...
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = self.data[i][j] + (other.data[i][j] - self.data[i][j]) * amount;
            }
        }
        v
    }
    ...
    pub fn column(&self, index: usize) -> Vector<T, R> {
        Vector::new_from_slice(self.data[index])
    }
    ...
    pub fn as_slice(&self) -> &[T] {
        unsafe { std::slice::from_raw_parts(&self.data as *const [[T; R]; C] as *const T, R * C) }
    }

    #[inline]
    pub fn as_slice_mut(&mut self) -> &mut [T] {
        unsafe {
            std::slice::from_raw_parts_mut(&mut self.data as *mut [[T; R]; C] as *mut T, R * C)
        }
    }
        ...
        T: NumericFloat,
    {
        ...
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = self.data[i][j].powf_numeric(other.data[i][j]);
            }
        }
        v
    }
        ...
        T: NumericFloat,
    {
        ...
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = self.data[i][j].floor_numeric();
            }
        }
        v
    }
        ...
        T: NumericFloat,
    {
        ...
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = self.data[i][j].round_numeric();
            }
        }
        v
    }
        ...
        T: NumericFloat,
    {
        for i in 0..C {
            for j in 0..R {
                let diff = other.data[i][j] - self.data[i][j];
...
impl<T: NumericFloat> Matrix<T, 4, 4> {
    ...
    pub fn from_translation(translation: Vector<T, 3>) -> Self {
        Self {
            data: [
                ...
                translation.extend(T::ONE).data[0],
            ],
        }
    }
    ...
    pub fn extract_rotation_scale(&self) -> (Quaternion<T>, Vector<T, 3>) {
        ...
        let rotation = Matrix {
            data: [
                [
                    self.data[0][0] / scale[0],
                    self.data[0][1] / scale[0],
                    self.data[0][2] / scale[0],
                    T::ZERO,
                ],
                [
                    self.data[1][0] / scale[1],
                    self.data[1][1] / scale[1],
                    self.data[1][2] / scale[1],
                    T::ZERO,
                ],
                [
                    self.data[2][0] / scale[2],
                    self.data[2][1] / scale[2],
                    self.data[2][2] / scale[2],
                    T::ZERO,
                ],
        ...
        T: NumericFloat + Neg<Output = T>,
    {
        ...
        let u = r.cross(f);
        Self::looking_at_create_matrix(r, u, f, from)
    }
    ...
    for Matrix<T, R, C>
{
    ...
    fn try_from(value: &[T; V]) -> Result<Self, Self::Error> {
        ...
        for c in 0..C {
            let end = offset + R;
            v.data[c] = value[offset..end].try_into()?;
            offset = end;
        }
        Ok(v)
    }
...
impl<const ROWS: usize, const COLUMNS: usize> Matrix<bool, ROWS, COLUMNS> {
    pub const TRUE: Self = Self {
        data: [[true; ROWS]; COLUMNS],
    };
    pub const FALSE: Self = Self {
        data: [[false; ROWS]; COLUMNS],
    };
...
impl<T: PartialOrd, const ROWS: usize, const COLUMNS: usize> Matrix<T, ROWS, COLUMNS> {
    pub fn less_than_per_component(self, other: Self) -> Matrix<bool, ROWS, COLUMNS> {
        ...
        for c in 0..COLUMNS {
            for r in 0..ROWS {
                m.data[c][r] = self.data[c][r] < other.data[c][r]
            }
        }
        m
    }
...
// impl<T, U, const R: usize, const C: usize> From<Matrix<U, R, C>> for Matrix<T, R, C>
// where
//     T: From<U>,
// {
//     fn from(input: Matrix<U, R, C>) -> Self {
//         let mut output = Matrix::<T, R, C>::ZERO;
//         for i in 0..C {
//             for j in 0..R {
//                 output.data[i][j] = input.data[i][j].clone() as T;
//             }
//         }
//         output
//     }
...
// impl<T, U, const R: usize, const C: usize> From<Matrix<U, R, C>> for Matrix<T, R, C> {
//     fn from(input: Matrix<f32, R, C>) -> Self {
//         let mut output = Matrix::<T, R, C>::ZERO;
//         for i in 0..C {
//             for j in 0..R {
//                 output.data[i][j] = input.data[i][j] as T;
//             }
//         }
//         output
//     }
...
impl<T: Numeric + Clone, const R: usize, const C: usize> Matrix<T, R, C> {
    pub fn select(mask: Matrix<bool, R, C>, if_true: Self, if_false: Self) -> Self {
        let mut output = Self::default();
        for i in 0..C {
            for j in 0..R {
                output.data[i][j] = if mask.data[i][j] {
                    if_true.data[i][j].clone()
                } else {
                    if_false.data[i][j].clone()
                };
            }
        }
        output
...
// impl<T: NumericFloat, const R: usize, const C: usize> Matrix<T, R, C> {
//     pub fn to_bits(&self) -> Matrix<u32, R, C> {
//         let mut v = Matrix::<u32, R, C>::ZERO;
//         for i in 0..C {
//             for j in 0..R {
//                 v.data[i][j] = self.data[i][j].to_bits()
//             }
//         }
//         v
//     }
...
impl<T, const R: usize, const C: usize> Matrix<T, R, C> {
    pub fn map<V: Numeric>(&self, mut f: impl FnMut(&T) -> V) -> Matrix<V, R, C> {
        let mut v = Matrix::<V, R, C>::default();
        for i in 0..C {
            for j in 0..R {
                v.data[i][j] = (f)(&self.data[i][j])
            }
        }
        v
    }
